...
import CarouselView from '../c2components/carousel';
...
function C2IssueScreen({navigation}) {
  ...
  async function collectData() {
    let record = await pb.collection('issues2').getOne(id);
    setData(record);
  }
  ...
  async function pushNotify() {
    const response = await fetch('http://68.178.168.6:8080/resolved2');
  ...
  async function submit() {
    ...
    const record = await pb.collection('issues2').update(data.id, formData);
    if (record) {
      pushNotify();
      handleModalHide3();
      handleModalShow4();
